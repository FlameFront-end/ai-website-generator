      ...
      <dl className={className}>
        ...
        {spec.metrics && spec.metrics.length > 0 && (
          <div>
            <dt>Метрики</dt>
            <dd>
              {renderInlinePairs(spec.metrics)}
            </dd>
          </div>
        )}
        ...
        {spec.productCard && (
          <>
            ...
            {spec.productCard.sections?.map((section, index) => (
              <div key={index}>
                <dt>Карточка · {section.title || section.type}</dt>
                <dd>
                  {section.content}
                  {section.details &&
        ...
        {spec.floatingCards && spec.floatingCards.length > 0 && (
          <div>
            <dt>Мини-карточки</dt>
            <dd>
              {renderInlinePairs(spec.floatingCards)}
            </dd>
          </div>
        )}
...
function renderInlinePairs(items: Array<{ value?: string; label?: string }>) {
  return items.map((item, index) => (
    <span key={`${item.value ?? ""}-${item.label ?? ""}-${index}`}>
